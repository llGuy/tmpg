...
namespace net {
    ...
    uint16_t Server::ParseUDPMessage(Byte* data, uint32_t size, float gravity, tmpg::EntitiesHandler& eh, tmpg::Platform& platform) 
    {
	...
	switch (type)
	{
	case CLIENT_UPDATE: return ParseClientUpdate(parser, gravity, eh, platform);
	};
    }

    // need to update the state of the client
    uint16_t Server::ParseClientUpdate(PacketParser& parser, float gravity, tmpg::EntitiesHandler& eh, tmpg::Platform& platform) 
    {
	...
	for (;;)
	{
	    ...
	    // parse message
	    auto player = ParseUDPMessage(messageBuffer, pair.second, ph.Gravity(), eh, platform);

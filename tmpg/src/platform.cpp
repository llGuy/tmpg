...
namespace tmpg {
	...
	// height and normal
	std::pair<float, glm::vec3> Platform::HeightAtPoint(float x, float z)
	{
		...
		if (x > dmxf || x < -dmxf || z > dmzf || z < -dmzf) return { -200.0f, normal };
		...
		case action_t::START_TERRAFORMING:
			if (entity.Terraforming() == -1)
			{
				for (Ray ray(entity.Direction(), entity.EyePosition(), 0.25f, 6.0f); ray.DistanceCovered() < ray.DistancetoEnd(); ray.Extend())
				{
					...
					float terrainHeightatRay = HeightAtPoint(currentPosition.x, currentPosition.z).first;
